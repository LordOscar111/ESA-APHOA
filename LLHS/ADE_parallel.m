function  [pop,fitness,Gbest]=ADE_parallel(pop,fitness,N,lb,ub, dim, MaxIt,gen,Gbest,funcid)%
...
%% Initialization
pop1=struct('Position',zeros(1,dim),'Cost',inf);
pop2=struct('Position',zeros(1,dim),'Cost',inf);
Pop1Size=round((((NPopmin-NPopinit)/MaxIt)*(it-1)) + NPopinit);
Pop2Size=N-Pop1Size;
pop1_flag=inf;
pop2_flag=inf;
p1=1;
...
for i=1:N
    if i<=Pop1Size
        pop1(i).Position=pop(i,:);
        % Evaluation
        pop1(i).Cost =fitness(i);
        if pop1(i).Cost<pop1_flag
            pop1_flag=pop1(i).Cost;
            p1=i;
        end
        ...
        j=i-Pop1Size;
        pop2(j).Position=pop(i,:);
        pop2(j).Cost =fitness(i);
        if pop2(j).Cost<pop2_flag
            pop2_flag=pop2(j).Cost;
            p2=i;
        end
    end
end

BestSol1Pop1.Position=pop(p1,:);
BestSol2Pop1.Position=pop(p2,:);

BestSol1Pop1.Cost=fitness(p1);
BestSol2Pop1.Cost=fitness(p2);
...
BestSol.Position=Gbest.Position;
BestSol.Cost=Gbest.Cost;
...
%     end
    ...
    for i=1:Pop1Size
        ...
        % Evaluation
        NewSol.Position=z1;
        NewSol.Cost= benchmark_func(NewSol.Position', funcid);
    ...
    for j=1:Pop2Size
        ...
        % Evaluation
        NewSol2.Position=z2;
        NewSol2.Cost= benchmark_func(NewSol2.Position', funcid);
...
for i=1:Pop1Size
    ...
    if i<=Pop1Size
        pop(i,:)=pop1(i).Position;
        % Evaluation
        fitness(i)=pop1(i).Cost;

    else
        j=i-Pop1Size;
        pop(i,:)=pop2(j).Position;
       fitness(i)=pop2(j).Cost ;
...
Gbest.Position=BestSol.Position;
Gbest.Cost=BestSol.Cost;
