...
function [X,fitness_f,Gbest,global_fitness,global_position]=COA_parallel(pop_size,gen,max_gen,lb,ub,dim,funcid,X,fitness_f,Gbest,global_fitness,global_position)
...
T=max_gen;
N=pop_size;
Best_fitness=Gbest.Cost;
best_position=Gbest.Position;

if isempty(global_fitness)
...
for i = 1:N
    if temp>30
        ...
        %% foraging stage
        if flag==1
            food_fit = benchmark_func(Xfood', funcid);
            flag=0;
        end

        P = 3*rand*fitness_f(i)/food_fit; %Eq.(4)
...
for i =1:N
    ...
    new_fitness = benchmark_func(Xnew(i,:)', funcid);
...
Gbest.Cost=Best_fitness;
Gbest.Position=best_position ;
end
