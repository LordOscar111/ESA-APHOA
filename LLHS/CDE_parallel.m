function [p,objF,Gbest]=CDE_parallel(p,objF,lb,ub,gen,maxGen,Gbest,funcid)
...
trial_fitness=zeros(1,popsize);

for i=1:popsize
    trial_fitness(i) = benchmark_func(trial(i,:)', funcid);
    if  trial_fitness(i)<objF(i)
      
        p(i,:)=trial(i,:);
        objF(i)=trial_fitness(i);
        if trial_fitness(i)<Gbest.Cost
