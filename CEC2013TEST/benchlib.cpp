    ...
    mclOutputHandlerFcn print_handler)
{
    ...
    if (!mclmcrInitialize())
        return false;
    if (!GetModuleFileName(GetModuleHandle("benchlib"), path_to_dll, _MAX_PATH))
        return false;
    {
...
{
    return benchlibInitializeWithHandlers(mclDefaultErrorHandler, mclDefaultPrintHandler);
}
...
{
    return mclFeval(_mcr_inst, "benchmark_func", nlhs, plhs, nrhs, prhs);
}

LIB_benchlib_CPP_API 
void MW_CALL_CONV benchmark_func(int nargout, mwArray& fit, const mwArray& x, const 
                                 mwArray& func_num)
{
    mclcppMlfFeval(_mcr_inst, "benchmark_func", nargout, 1, 2, &fit, &x, &func_num);
}
